...
contract BumpUpkeep is ILogAutomation {
    event Bumped(
        address indexed logSender,
        address indexed upkeepSender, 
    ...
    uint256 public selectedTokenId;
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        uint256 tokenId = bytes32ToUint(log.topics[2]);

        console.log("TokenId: %s %d", tokenId, tokenId);

        performData = abi.encode(logSender, number, log.txHash, tokenId);
        return (true, performData);
    }

    function performUpkeep(bytes calldata performData) external override {
        (address logSender, uint256 number, bytes32 txnHash, uint256 tokenId) = abi.decode(
            performData,
        ...
        console.log("TokenId from upkeep: %s %d", tokenId, tokenId);
        console.log("Selected TokenId: %s %d", selectedTokenId, selectedTokenId);

        emit Bumped(logSender, msg.sender, number, block.number, counter, txnHash);
    ...
    function upgrade(uint256 _tokenId) public {
        uint256 newValue = grades(_tokenId) + 1;
        string memory newUri = tokenUris[newValue];
        _setTokenURI(_tokenId, newUri);
    }
   
    function grades(uint256 _tokenId) public view returns (uint256) {
