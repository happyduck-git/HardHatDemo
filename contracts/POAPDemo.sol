...
contract POAPDEMO is ERC721Enumerable, ERC721URIStorage, Ownable, AutomationCompatible {
    ...
    function checkUpkeep(bytes calldata /*checkData*/) external view override returns (bool upkeepNeeded, bytes memory performData) {
        ...
        if (mintNeeded == 0) {
            performData = abi.encode(mintNeeded);
    ...
    function performUpkeep(bytes calldata performData) external {
        ...
        (uint256 passedValue) = abi.decode(
            performData,
            (uint256)
        );
        
        mintNeeded = passedValue + 99;
